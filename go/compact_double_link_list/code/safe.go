...
func exampleSafe() {
	...
	list.InsertNextItem(itemX, itemY)
	list.InsertPreviousItem(&cdllist.ListItem{Data: "W"}, itemX)
	fmt.Println(list.EnlistAllItemValues())

	list.MoveItemToHeadPosition(itemX)
	list.MoveItemToTailPosition(itemX.GetNextItem().GetNextItem())
	fmt.Println(list.EnlistAllItemValues())

	list.MoveItemToBeforeReference(itemY, itemX)
	// 'K' does not exist, so this Move does not happen.
	list.MoveItemToAfterReference(itemX, &cdllist.ListItem{Data: "K"})
	fmt.Println(list.EnlistAllItemValues())

	list.MoveItemToAfterReference(itemX, list.GetTail().GetPreviousItem())
	fmt.Println(list.EnlistAllItemValues())
	...
	list.InsertNextItem(itemX, itemZ)
	fmt.Println(list.EnlistAllItemValues())

	list.RemoveItem(itemZ)
	fmt.Println(list.EnlistAllItemValues())
